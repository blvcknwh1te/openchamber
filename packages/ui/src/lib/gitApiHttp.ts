...
const invalidateGitStatusCache = (directory: string, runtimeKey = getRuntimeKey()): void => {
    ...
    if (operation.state !== 'planned' && operation.state !== 'running') {
      if (!tracked.value.terminalState) {
        if (shouldInvalidateForTerminalOperation(operation)) {
          invalidateGitStatusCache(tracked.value.directory, tracked.value.runtimeKey);
        }
