...
`session-last-model.ts` projects the last model each session ran on out of the global session cache, because a row cannot read it from the node it renders: the tree is rebuilt only for structural session changes, and the server records a new model against a recency-only `time.updated`. Each row subscribes to its own session ID, so the badge appears without a history request and without a tree rebuild. The index mirrors `entityById` exactly, including the runtime-switch reset, and keeps the projected object of every unchanged session so a read does not re-render that row.
...
Initial loads use smaller pages on constrained VS Code/mobile surfaces. Prefetch resolves only the initial renderable page; it does not eagerly download older history. A materialized snapshot short-circuits that initial load only when the loader already knows the session's coverage (`complete` or an older-page cursor): records in the store prove the transcript can render, never that it is fully loaded, so an unknown coverage still fetches the initial page and registers the coverage. The mounted chat timeline requests older pages when its viewport is underfilled or the user scrolls toward history, while mobile uses its explicit load-older action. The subtask dialog hosts the same list without the chat timeline controller, so it applies those two rules itself through `shouldAutoLoadEarlierForUnderfilledPinnedViewport` and `resolveHistoryScrollThreshold`. Timeline caches, pending work, prepend snapshots, and stale checks use runtime + directory + session identity so equal session IDs in different worktrees cannot share lifecycle state. Older pages are fetched through the same loader and merged with optimistic records before publication. The same chronology contract applies in the VS Code webview because it consumes this shared loader and sync store; the extension bridge must transport OpenCode records without introducing its own ID-based ordering.
